<script lang="ts">
	...
	if (gear?.stats) {
		for (let i = 0; i < gear.stats.length; i++) {
			if (
				gear.stats[i].type.name == MainStats.AGILITY ||
				gear.stats[i].type.name == MainStats.STRENGTH ||
				gear.stats[i].type.name == MainStats.INTELLECT
			) {
	...
	function getWowheadLink(gear: EquippedItem): string {
		const enchant = gear.enchantments?.[0]?.enchantment_id ?? "";

		const gems =
			gear.sockets
				?.filter((socket) => socket.item)
				.map((socket) => socket.item!.id)
				.join(":") ?? "";

		return `https://www.wowhead.com/item=${gear.item.id}&ilvl=${gear.level.value}&ench=${enchant}&gems=${gems}`;
	}
</script>
	...
	<Tooltip.Provider>
		<Tooltip.Root>
			<Tooltip.Trigger>
				<div
					class="gear-slot"
					style="--quality-color: {itemRarityColor(gear.quality.type)};"
				>
					<a
						target="_blank"
						href={getWowheadLink(gear)}
					...
					>
						...
						{#if gear.enchantments && gear.enchantments.length > 0}
							<img
								class="w-5 absolute -top-1 -right-1"
								src={enchant_scroll}
								alt="Enchant Scroll"
							/>
						{/if}
						...
						{#if gear.sockets?.find((s) => s.item != undefined)}
							<img
								class="w-4 absolute -top-1 -left-1"
								src={prismatic_socket}
								alt="Prismatic Socket"
							/>

							{#if gear.sockets.length > 1}
								<img
									class="w-4 absolute top-4 -left-1"
									src={prismatic_socket}
									alt="Prismatic Socket"
								/>
							{/if}
			...
			{#if !appSettings.settings.useWowheadTooltips}
				<Tooltip.Content class="bg-transparent">
					<div
						class="tooltip-content"
						style="--rarityColor: {itemRarityColor(gear.quality.type)};"
					>
						<!-- Item Header -->
						<div class="mb-3">
							<p
								class="text-lg font-bold mb-1"
								style="color: var(--rarityColor);"
							>
								{gear.name}
							</p>

							<p class="text-sm text-gray-300 mb-1">
								{gear.level.display_string}
							</p>

							<p class="text-sm text-gray-400">
								{gear.inventory_type.name}
							</p>
						</div>
						...
						<div class="space-y-1">
							{#each gear.stats as stat}
								...
								{:else if stat.type.name !== "Agility" &&
									stat.type.name !== "Strength" &&
									stat.type.name !== "Intellect"}
